...
class ModelTrainer:
    ...
    def use_rw(self, model):
        ...
        _, weights = rw.fit_transform(data.drop(self.label, axis=1), data[self.label])
        if self.params.get("scaler"):
            model.fit(
                self.dataset.drop(columns=self.label),
    ...
    def use_eg(self, model):
        ...
        eg_model.fit(
            self.dataset.drop(columns=self.label),
            self.dataset[self.label],
            sensitive_features=self.dataset[self.sensitive_features],
        )
        return eg_model
    ...
    def use_prj(self):
        ...
        prej = PrejudiceRemover(
            sensitive_attr=self.sensitive_features[0], class_attr=self.label
        )
        prej.fit(bin_data)
        return prej
    ...
    def use_rej_opt(self, model):
        ...
        rej = RejectOptionClassifierCV(
            prot_attr=self.sensitive_features[0], scoring="statistical_parity"
        )
